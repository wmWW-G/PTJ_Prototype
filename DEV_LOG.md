...
## 2026-07-23：模板宽版交互与个人模板命名

- 自定义单图编辑在用户尚未点击生成时只展示原图，不再提前显示静态“参考预览”；开始生成后才出现生成中区域，真实结果返回后再展示 AI 新版本。
- “我的模板”卡片改为单击选中，不再直接跳入自定义编辑器；补齐与预设模板一致的“查看详情”，可检查整套职责与保存过的真实候选图。
- 个人模板被选中后可直接点击底部“使用此模板”，并恢复保存时的整套职责配方；原始“自定义套图/详情图”入口仍需点击“继续编辑”才进入编辑页。
- 所有预设模板的生图维度改为常驻显示，选中时只改变边框与勾选，不再增删内容造成卡片跳动；卡片统一拆为弹性内容区和固定底部操作区，使个人模板的“查看详情”与同排其他模板对齐。
- “我的模板”补回独立的“继续编辑”入口：点击卡片仍只负责选中，点击编辑才恢复保存时的整套职责、已采用图片、原修改指令和槽位状态；再次保存会覆盖更新原模板并保留“我的模板01”名称，不会误创建新编号。

## 2026-07-23：高信息密度商品信息图模板

- 将九套视觉模板全部统一为 high 密度契约：单张 9–12 个信息单元、至少 4 个辅助视觉、5–8 条画面文案、80% 目标占比；极简和生活方式只保留风格差异，不再允许少文字或大留白。
- 新增 8 个仅服务器登记的布局 recipe，并把自定义候选图采用操作绑定到固定 `layout_recipe_id`；个人模板只持久化白名单 ID，旧 v1 无字段记录继续可读，未知 ID 不会进入生成请求。
- Planner 对 high 模板拒绝低密度首份计划并进行一次修复；硬信息仅允许逐字使用已确认资料，描述标签必须来自用户事实、商品上下文或视觉证据。
- 每张图新增统一的参考图级最低框架：醒目标题、解释副标题、主商品或主场景，以及至少 4 个“图片 + 短标签 + 一句解释”模块。该框架同时进入 Planner 的逐张职责和编排器最终图片模型 Prompt，确认计划也不能绕过。
- 使用内置 ImageGen 按用户最新参考图重新生成九套模板的 62 张独立预览，统一保存到 `public/demo/templates-v2/`；三份 `manifest-*.json` 记录每张提示词、参考图和验收口径。自动测试检查 62 条唯一路径、真实文件和唯一内容摘要，禁止重复占位图。
- 主图、套图、详情图和海报统一支持 1 张独立风格参考图；它始终进入 `style_reference_assets`，不参与商品主体分析，商品素材保持在 `reference_assets`。仅风格图同样触发图生图。
- Prompt 审核显示信息单元数量摘要；当前图片文字仍由图片模型生成，精确叠字尚未完成，未对文字、数字、认证或版式准确率作承诺。
- 最终验证通过：后端 `66 passed`，前端 `65 passed`，`npm run lint`、`npm run build` 与 `git diff --check` 均通过；62 张预览全部为 1024×1024 JPEG，内容摘要 62/62 唯一，三份 manifest JSON 有效且登记路径全部存在。
- 真实 Chromium 已切到本地最新 `/api/capabilities` 验收：套图五套、详情图四套均显示“高信息量”；模板卡当前加载的 44 个预览元素全部 `naturalWidth > 0`，路径均来自 `demo/templates-v2/`，浏览器 `warn/error` 日志为空。页面停留在详情图模板选择层，便于继续人工查看。

## 2026-07-22

- 修复自定义单图点击“采用这张”后个人模板无法真正保存的问题：保存动作现在会把图片 URL、修改指令、槽位和整套职责顺序写入 `ptj.prototype.personal-templates.v1`，刷新后仍可从“自定义”分类打开。
- 个人模板存储会校验图片类型、槽位、职责数量和必要字段；LocalStorage 失败时显示可重试错误，不会伪装成已保存。当前边界仍为本机原型存储，不会跨账号或跨设备同步。
- 按页面标注移除单图编辑器底部的“继续修改”和“再生成一个”，保留输入区内的“重新生成”与底部唯一主动作“采用这张”。
- 将真实批量生图改为“两阶段确认”：第一次请求独立的 `POST /api/generations/plan`，后端强制 `planning_only=true`，只调用 Gemini Prompt Planner 并逐版返回结构化 Prompt；前端确认前不会调用图片模型，也不会创建历史生图任务。
- 新增简洁的右侧“确认生图 Prompt”面板，按方案和图片职责逐张展示完整 Prompt；整套一致性要求默认折叠，主操作集中为单张修改与“确认 Prompt，开始生成”。
- 支持针对任意一张输入改进意见，通过 `/api/generations/refine-prompt` 让 LLM 只重写该张正向 Prompt、负向 Prompt 和可见文案；序号、职责和标题由后端保留，避免破坏整套结构。
- 用户确认后，前端通过 `confirmed_plans` 原样提交最终方案；后端严格校验槽位数量和顺序，恢复服务器可信的 role/title，并跳过商品分析和 Prompt Planner，防止生图前再次漂移。
- 新增 Vercel 精确函数入口 `api/generations/plan.py` 和 `api/generations/refine-prompt.py`；规划与真实生图使用不同 URL，新前端连接旧后端时只会安全地得到 404，不会因旧后端忽略新参数而误生图。
- 新增只规划不调用图片供应商、独立规划路由强制安全模式、确认计划跳过 Planner、单张 LLM 重写、HTTP 接口和前端确认交互测试；后端全量 58 项、前端全量 56 项、TypeScript 检查、Vite Production 构建及 `git diff --check` 通过。
- 本地页面无费用验收确认首要按钮已改为“生成 Prompt”，并明确提示确认后才消耗图片额度；本轮未点击按钮，没有调用线上 Planner 或图片模型。
- 将两阶段 Prompt 确认后端发布到 Vercel Production `dpl_BRBWqGgt3zgKQCz8WbiAJ73mzvhD`，并完成固定域名 `https://ptj-image-api.vercel.app` 切换，本地 Vite 无需修改 API 地址即可测试。
- 通过本地页面实测线上 Planner：标准套图正确返回 6 张结构化 Prompt，第 1 张按“轻微俯拍、保留橙白配色、柔和自然阴影”单独重写成功，其他 5 张保持不变；页面停在“确认 Prompt，开始生成 6 张”，未调用图片模型。
- 修复自定义模板单图编辑的参考图展示：附加图片现在只作为 GPT-Image-2 的额外输入，并显示在输入框下方的附件缩略图中；左侧对比区始终保留当前模板原图，不再被参考图覆盖。
- 自然语言修改框支持直接粘贴 1 张图片；图片粘贴与“附加参考图”共用格式校验和单图状态，普通文字粘贴仍按文本处理。
- 修复真实成图已经返回但界面迟迟不显示的问题：前端收到首个 `anchor_completed` / `image_completed` 成图事件后立即展示结果，同时在后台继续消费 NDJSON 尾部事件，兼顾即时反馈与服务端收尾。
- 新增参考图不覆盖原图、输入框粘贴图片和首张成图事件立即返回的回归测试；前端全量 54 项、TypeScript 检查、Vite Production 构建均通过。
- 本地页面完成无费用回归：上传参考图后左侧仍显示模板原图，附件缩略图和文件标签正常出现；本轮未点击“重新生成”，没有额外发起付费生图调用。
- 将自定义模板单图编辑从静态候选图切换为真实 GPT-Image-2：固定 `gpt_image_2_openrouter`、1:1、`1K + low`、单张主图请求，界面仅展示“GPT-Image-2 · 低”，不再增加模型参数选择。
- 自定义编辑器支持纯文字生图，也支持上传 1 张 PNG/JPG/WebP 参考图后再结合文字图生图；参考图沿用 `/api/uploads` 受控上传，结果沿用 `/api/generations/stream` NDJSON 链路，只有收到真实 `anchor_completed` / `image_completed` 才解锁“采用这张”。
- 增加上传缩略图、移除参考图、生成中状态、错误提示、真实结果 URL 渲染和重复生成；远程 Blob、Blob 预览与项目静态资源分别解析，避免 GitHub Pages 基础路径破坏真实图片地址。
- 明确当前边界：真实单图生成已经接通；“提取模板特征并持久化到我的模板”仍是前端原型状态，界面不再把它描述为已真实完成。
- 新增固定模型请求、附图上传、NDJSON 成图返回和页面交互测试；前端全量 51 项、TypeScript 检查、Vite Production 构建及 `git diff --check` 通过。
- 在本地页面使用用户提供的帽子参考图完成一次真实“附图 + 文字”验收：页面先显示 GPT-Image-2 生成中，随后加载 Vercel Blob 结果，实际 PNG 为 1254×1254，“采用这张”成功解锁；未重复发起第二次付费调用。
- 优化品牌 Logo 上传交互：图片通过格式和大小校验并成功加入任务后，自动收起“添加品牌 Logo”设置面板；无效文件仍保留面板，便于用户直接重试。
- 增加前端回归测试，固定检查 Logo 上传成功后的面板关闭状态和“Logo 已添加”入口状态。
...
# 2026-07-22：单图 AI 修改与模板特征提取原型

- 把“自定义套图 / 自定义详情图”升级为大尺寸右侧抽屉；整套先使用 3 列图片预览，用户点击任一张后进入单图修改。
- 单图界面只保留原图、AI 新版本、一个自然语言修改框，以及继续修改、再生成、采用三个动作；营销文案由 AI 生成，不再要求用户逐项填写。
- 用户采用候选图后才启用“保存为我的模板”，界面明确提示 AI 将提取构图、信息结构和文案规则；认证、MOQ、材质等数据只能来自已确认资料。
- 当前重新生成、采用和保存均为可交互前端原型状态，尚未接入真实单图生成或视觉配方持久化接口，页面不会把演示素材伪装成真实后端结果。
- 新增 3 张用户参考棒球帽素材与 2 项组件回归场景；前端 48 项测试、TypeScript 检查和 Vite Production 构建通过。
- 1440 × 1024 桌面稿与 700 × 900 窄屏均完成浏览器验收，控制台无 error / warning；对照记录见 `design-qa.md`。
# 2026-07-23：模板宽版交互与个人模板命名

- 套图和详情图的模板选择抽屉统一扩大为与自定义编辑相同的宽版布局，并移除顶部模板分类标签。
- 自定义卡改为“点击卡片仅选中、点击继续编辑才进入编辑页”，避免误触后直接跳转。
- 预设模板详情统一使用自定义模板的整套网格展开方式；6 张使用 3×2，8 张详情图使用 4×2，减少纵向滚动。
- 压缩整套预览和单图 AI 编辑的标题、缩略图、对比图及操作区高度，使常见桌面视口可在一屏完成主要操作。
- “使用这套模板”改为“保存模板”；保存后的个人模板按 `我的模板01`、`我的模板02` 自动顺序命名，并兼容旧版浏览器记录。
- 本次修改共享 `VisualTemplatePicker`，因此同时覆盖套图与详情图。
