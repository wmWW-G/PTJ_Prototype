...
const modeNames: Record<GenerationMode, string> = {
  "text-to-image": "批量文生图", "image-to-image": "批量图生图",
...
export function HistoryDetailPage() {
  ...
  if (!task) return <div className={styles.empty}><ImageOff size={36} /><h1>没有找到这条生成记录</h1><p>记录可能已被清理，你可以返回任一生成页面重新创建。</p><Link to="/text-to-image">返回批量文生图</Link></div>;
  ...
  return (
    <article className={styles.detail}>
      ...
      <header><div><span>{modeNames[task.mode]}</span><h1>{new Date(task.createdAt).toLocaleString("zh-CN", { hour12: false })}</h1><p>{task.prompt}</p></div><div className={styles.meta}><b>图片模型 · {task.model}</b><b>图片尺寸 · {task.aspectRatio}</b></div></header>
