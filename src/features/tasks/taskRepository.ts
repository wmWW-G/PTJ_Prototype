...
function cloneDemoTasks(): GenerationTask[] {
  return DEMO_TASKS.map((task) => ({
    ...task,
    customVisualRoles: (task.customVisualRoles ?? []).map((role) => ({ ...role })),
    styleImages: [...(task.styleImages ?? [])],
...
function normalizeTask(task: Partial<GenerationTask> & Pick<GenerationTask, "id" | "mode" | "prompt" | "imageType" | "createdAt">): GenerationTask {
  ...
  return {
    ...
    customVisualRoles: (task.customVisualRoles ?? []).map((role) => ({ ...role })),
    supplementalInfo: task.supplementalInfo ?? {},
    ...
    styleImages: task.styleImages ?? [],
    sourceImages: task.sourceImages ?? [],
    logoImage: task.logoImage,
    logoPosition: task.logoPosition ?? "bottom-right",
    modelImages: task.modelImages ?? [],
    garmentImages: task.garmentImages ?? [],
    resultImages: task.resultImages ?? [],
...
export function createMockTask(input: CreateTaskInput): GenerationTask {
  return {
    ...
    visualTemplateId: input.visualTemplateId ?? "standard_product",
    customVisualRoles: (input.customVisualRoles ?? []).map((role) => ({ ...role })),
    supplementalInfo: input.supplementalInfo ?? {},
    ...
    logoPosition: input.logoPosition ?? "bottom-right",
